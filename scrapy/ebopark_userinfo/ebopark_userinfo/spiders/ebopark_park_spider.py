...
class ParkSpider(Spider):
    ...
    start_urls = ['http://cees.ebopark.com/parkinfo/list?park_code=&park_name=&is_monthlyrent=ALL&is_temppay=ALL&is_duty=ALL&is_video=ALL&is_cloud=ALL&is_elecreceipt=ALL&is_cpgs=ALL&time_type=no&start_time=&end_time=&park_type=ALL&addr=&page=1&_=1695000542137']

    csv_file_path = "park-all.csv"

    current_page = 1
    max_page = 130

    def start_requests(self):
        url = "http://cees.ebopark.com/parkinfo/list?park_code=&park_name=&is_monthlyrent=ALL&is_temppay=ALL&is_duty=ALL&is_video=ALL&is_cloud=ALL&is_elecreceipt=ALL&is_cpgs=ALL&time_type=no&start_time=&end_time=&park_type=ALL&addr=&page=%d&_=1695000542137"%(self.current_page)
    ...
    def ebo_parse_page(self, response):
        ...
        if len(data_list) == 24:
            self.current_page += 1
            next_url = "http://cees.ebopark.com/parkinfo/list?park_code=&park_name=&is_monthlyrent=ALL&is_temppay=ALL&is_duty=ALL&is_video=ALL&is_cloud=ALL&is_elecreceipt=ALL&is_cpgs=ALL&time_type=no&start_time=&end_time=&park_type=ALL&addr=&page=%d&_=1695000542137"%(self.current_page)
