...
class ParkSpider(Spider):
    ...
    start_urls = ['http://cees.ebopark.com/company?name=&page=1']

    csv_file_path = "company-all.csv"

    current_page = 1
    max_page = 130

    def start_requests(self):
        url = "http://cees.ebopark.com/company?name=&page=%d"%(self.current_page)
    ...
    def ebo_parse_page(self, response):
        data = response.json()
        code = data['code']
        current_page = data['data']['list']['current_page']
        data_list = data['data']['list']['data']
        print(f"============================> Code: {code}")
        ...
        if len(data_list) == 24:
            self.current_page += 1
            next_url = "http://cees.ebopark.com/company?name=&page=%d"%(self.current_page)
